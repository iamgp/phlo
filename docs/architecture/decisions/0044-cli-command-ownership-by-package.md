...
- `phlo-quality`: `cli_validate.py`, `cli_schema.py`
- `phlo-alerting`, `phlo-lineage`, `phlo-metrics`, `phlo-postgrest`, `phlo-hasura`, `phlo-openmetadata`
...
- Ingestion + Quality
  ...
  - Move Pandera schema/test scaffolding pieces from `src/phlo/cli/scaffold.py` into `packages/phlo-quality` (or shared scaffold helper).
