...
**Note**: This publishes to Postgres (external_database), not Iceberg.
...
def publish_glucose_marts(context, trino: TrinoResource, postgres: PostgresResource) -> None:
    """
    Publish Iceberg gold tables to Postgres marts.
    
    Materialization:
    1. Query Iceberg gold tables
    2. Insert into Postgres marts (truncate-replace)
    3. Enable BI tools (Superset) to query marts
    """
    
    context.log.info("Publishing glucose metrics to Postgres")
    
    # Query Iceberg
    glucose_overview = trino.execute("""
        SELECT * FROM iceberg.gold.mrt_glucose_readings
        WHERE reading_date >= CURRENT_DATE - INTERVAL '90' DAY
    """).to_pandas()
    
    glucose_hourly = trino.execute("""
        SELECT * FROM iceberg.gold.mrt_glucose_hourly_patterns
        WHERE reading_date >= CURRENT_DATE - INTERVAL '90' DAY
    """).to_pandas()
    
    # Truncate and insert (idempotent)
    with postgres.cursor() as cursor:
        cursor.execute("TRUNCATE TABLE marts.mrt_glucose_overview")
        cursor.executemany("""
            INSERT INTO marts.mrt_glucose_overview
            (reading_date, avg_glucose_mg_dl, min_glucose_mg_dl, ...)
            VALUES (?, ?, ?, ...)
        """, glucose_overview.values.tolist())
        
        cursor.execute("TRUNCATE TABLE marts.mrt_glucose_hourly_patterns")
        cursor.executemany("""
            INSERT INTO marts.mrt_glucose_hourly_patterns
            (reading_date, hour, avg_glucose_mg_dl, ...)
            VALUES (?, ?, ?, ...)
        """, glucose_hourly.values.tolist())
    
    context.log.info(f"Published {len(glucose_overview)} daily records")
    context.log.info(f"Published {len(glucose_hourly)} hourly records")
```
