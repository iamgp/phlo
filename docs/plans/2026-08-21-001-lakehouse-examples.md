...
|---:|---|---|---|---|---|---|---|---|:---:|:---:|:---:|:---:|:---:|:---:|---|
| 1 | Retail Files | In progress | Technical/domain: `ingestion/retail`, shared `schemas`, `quality`, `schedules`, and central dbt | DLT; per-store/day CSV sales, JSON product/store/promotion references, NDJSON inventory, Parquet archive | Sales facts, product/store dimensions, append-ledger inventory deduplication, daily store marts, category performance, stockout/reorder | Malformed files, duplicates, missing store files, referential integrity, revenue reconciliation, native dbt check evidence, sequential WAP replay | Blessed Iceberg stack | A network-free golden path; replay is idempotent and bad partitions do not promote | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | Live E2E produced 12 Iceberg tables from 25 stores, 500 products, 60,000 generated sales rows, and 375,000 inventory snapshots. Five schedules and differentiated asset contracts verified. Two-partition WAP backfill promoted sequentially; dbt dimensions/facts/marts emitted native checks and promoted. Consumer is temporarily pinned to unmerged [#761](https://github.com/phlohouse/phlo/pull/761); mark Complete after review, merge, and revalidation from GitHub `main`. Terminal failed-report persistence is tracked in a focused follow-up PR thread. |
