...
|---:|---|---|---|---|---|---|---|---|:---:|:---:|:---:|:---:|:---:|:---:|---|
| 1 | Retail Files | Complete | Technical/domain: `ingestion/retail`, shared `schemas`, `quality`, `schedules`, and central dbt | DLT; per-store/day CSV sales, JSON product/store/promotion references, NDJSON inventory, Parquet archive | Sales facts, product/store dimensions, append-ledger inventory deduplication, daily store marts, category performance, stockout/reorder | Malformed files, duplicates, missing store files, referential integrity, revenue reconciliation, native dbt check evidence, sequential WAP replay | Blessed Iceberg stack | A network-free golden path; replay is idempotent and bad partitions do not promote | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | E2E revalidated from GitHub `main` at `d1ae216`: 12 Iceberg tables from 25 stores, 500 products, 60,000 generated sales rows, and 375,000 inventory snapshots. Five schedules and differentiated asset contracts verified. Two-partition WAP backfill promoted sequentially; dbt dimensions/facts/marts emitted native checks and promoted. A missing-partition run exhausted its configured retries, terminalized its WAP report, and left `main` unchanged. See the [case study](../../examples/lakehouses/retail-files/docs/retail-files-e2e.md). |
