...
**Severity:** High
**Exception Class:** `CascadeDiscoveryError`

## Description

This error occurs when Dagster cannot discover your asset definitions. Cascade uses Python decorators like `@cascade_ingestion` to define assets, and Dagster needs to be able to find and load these definitions.
...
2. **Incorrect decorator usage**
   - Missing `@cascade_ingestion` decorator
...
Ensure your asset is imported in `src/cascade/definitions.py`:

```python
# definitions.py
from cascade.defs.ingestion.weather.observations import weather_observations_asset
...
from cascade.defs.ingestion.weather.observations import build_defs
...
```bash
python -c "from cascade.defs.ingestion.weather.observations import weather_observations_asset"
```
...
4. **Check for circular imports**
   ```bash
   python -m py_compile src/cascade/defs/ingestion/weather/observations.py
   ```

## Related Errors

- [CASCADE-005: Missing Schema](./CASCADE-005.md) - Schema not provided to decorator
- [CASCADE-002: Schema Mismatch](./CASCADE-002.md) - unique_key not in schema
...
- [Cascade Asset Creation Guide](../TESTING_GUIDE.md#creating-assets)
