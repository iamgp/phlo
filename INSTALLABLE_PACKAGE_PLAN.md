...
1. Install Cascade via `pip install phlo` or `uv add phlo`
...
1. **Package structure**: Already uses `src/phlo/` layout
2. **Build configuration**: Modern `pyproject.toml` with hatchling
3. **Entry point**: CLI already defined (`phlo` command)
...
2. **Workflow location**: Currently in `src/phlo/defs/ingestion/`
...
"templates" = "phlo/templates"
```

#### 1.2 User Project Structure (What Users Create)

**Generated by `phlo init`:**
```
my-data-project/
├── pyproject.toml              # Minimal: just phlo dependency
├── .env                        # User secrets
├── .env.example
...
```python
# In src/phlo/defs/ingestion/__init__.py
...
```python
# In phlo/framework/discovery.py
import importlib
import sys
...
```python
# In phlo/framework/definitions.py
from dagster import Definitions
from phlo.config import CascadeConfig
...
# Non-interactive
$ phlo init my-data-project --template basic
...
def init(project_name: str, template: str, force: bool):
    ...
    - Minimal pyproject.toml with phlo dependency
...
#### 3.2 Modified Command: `phlo create-workflow`

**Changes needed:**
1. Remove assumption about being in Cascade repo
2. Find user project root (look for `pyproject.toml` with phlo dependency)
3. Create files in `workflows/` instead of `src/phlo/defs/`
...
def _generate_asset_template(config: dict[str, Any]) -> str:
    """Generate asset for USER PROJECT (not in phlo package)."""
...
$ phlo dev --host 0.0.0.0 --port 3000
...
class CascadeConfig(BaseSettings):
    ...
    iceberg_warehouse: str = "s3://phlo-warehouse"
    ...
    postgres_db: str = "phlo"
    postgres_user: str = "phlo"
...
**Move to separate `phlo-services` repo or keep as optional:**
...
```bash
# Clone services repo
git clone https://github.com/phlo/phlo-services
cd phlo-services
...
Include `phlo-services` as optional extra:
```toml
...
```python
# In phlo/framework/resources/dbt.py
...
```python
# phlo/testing/helpers.py
from dagster import materialize
from phlo.framework.resources import build_test_resources
...
`src/phlo/defs/` to a separate user project.
...
```bash
pip install phlo>=1.0.0
```

### 2. Create New Project Structure

```bash
phlo init my-phlo-project --from-existing
```
...
**Before:**
```python
from phlo.defs.ingestion.myworkflow.myasset import myasset
from phlo.schemas.myschema import MySchema
```
...
- Import paths changed from `phlo.defs.*` to `workflows.*`
...
pip install phlo[all]
```

**From source (development):**
```bash
git clone https://github.com/phlo/phlo
cd phlo
uv sync
uv pip install -e .
```
...
def build_definitions() -> Definitions:
    ...
    if legacy_workflows.exists():
        logger.warning(
            ...
            "See migration guide: https://phlo.dev/docs/migration"
        )
...
### Phase 1: Package Separation ⬜
- [ ] Rename `phlo/defs/` to `phlo/framework/`
...
- [ ] Create new `phlo/framework/definitions.py`
...
- [ ] Create `phlo dev` command
- [ ] Add `phlo services` command group
...
1. **Naming:** Keep `phlo` or rename for clarity?
...
```
my-weather-pipeline/
├── pyproject.toml          # Just: dependencies = ["phlo"]
├── workflows/
│   ├── ingestion/
│   │   └── weather/
