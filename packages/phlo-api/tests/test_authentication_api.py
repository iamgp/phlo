...
def test_phlo_api_service_mounts_docker_socket_read_only_for_runtime_status() -> None:
    import yaml
    from importlib.resources import files
    ...
    assert "/var/run/docker.sock:/var/run/docker.sock:ro" in service_defn["compose"].get(
        "volumes", []
    )
