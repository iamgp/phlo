"""Provider-neutral Observatory v2 run read model."""
...
from phlo_api.observatory_api.v2_models import RunStatus, V2ResourceRef, V2Run
...
def _normalize_legacy_dagster_run(run: Mapping[str, Any]) -> V2Run:
...
def _normalize_asset_refs(asset_keys: Any) -> list[V2ResourceRef]:
    if not isinstance(asset_keys, Sequence) or isinstance(asset_keys, (str, bytes)):
        return []

    refs: list[V2ResourceRef] = []
    for asset_key in asset_keys:
        ...
        if asset_id:
            refs.append(V2ResourceRef(kind="asset", id=asset_id, label=asset_id))
    return refs
