...
"""
    ...
    def _build_service_config(
        ...
        if service.image:
            ...
            config["build"] = {
                "context": ".",
                "dockerfile": f"images/{service.name}/Dockerfile",
                "args": {"BASE_IMAGE": service.image},
            }
        elif service.build:
    ...
    def copy_service_files(
        ...
        for service in services:
            if service.image:
                destination = output_dir / "images" / service.name / "Dockerfile"
                destination.parent.mkdir(parents=True, exist_ok=True)
                destination.write_text(self._hardened_image_dockerfile(service))
    ...
    def _hardened_image_dockerfile(self, service: ServiceDefinition) -> str:
        """Return a generated wrapper that upgrades the upstream image OS layer."""
        if not service.image:
            raise ValueError(f"service {service.name!r} has no image to harden")
        user_directive = (
            "\n# hadolint ignore=DL3002\nUSER 0" if service.compose.get("user") == "0" else ""
        )
        return (
            """# Generated by Phlo. Keep the upstream entrypoint and service contract.\n# hadolint ignore=DL3006\nARG BASE_IMAGE\nFROM ${BASE_IMAGE}\n\n"""
            + user_directive
            + """\nSHELL [\"/bin/sh\", \"-c\"]\nRUN if [ "$(id -u)" -ne 0 ]; then \\\n        echo "Non-root upstream image; retaining upstream filesystem"; \\\n    elif command -v apk >/dev/null 2>&1; then \\\n        apk upgrade --no-cache; \\\n    elif command -v apt-get >/dev/null 2>&1; then \\\n        apt-get update; \\\n        apt-get dist-upgrade -y; \\\n        rm -rf /var/lib/apt/lists/*; \\\n    elif command -v microdnf >/dev/null 2>&1; then \\\n        microdnf update -y; \\\n        microdnf clean all; \\\n    elif command -v dnf >/dev/null 2>&1; then \\\n        dnf upgrade -y; \\\n        dnf clean all; \\\n    elif command -v yum >/dev/null 2>&1; then \\\n        yum update -y; \\\n        yum clean all; \\\n    else \\\n        echo \"No supported package manager in ${BASE_IMAGE}; retaining upstream filesystem\"; \\\n    fi\n"""
        )
