...
"""
    ...
    def _build_service_config(
        ...
        # Dev mode: inject phlo source mount and pyproject.toml for dependency sync
        if dev_mode and service.phlo_dev and phlo_src_path:
            # Mount phlo source code
            phlo_mount = f"{phlo_src_path}:/usr/local/lib/python3.11/site-packages/phlo:ro"
            config["volumes"].append(phlo_mount)
            # Mount pyproject.toml for dependency sync (two levels up from phlo source)
            pyproject_mount = (
                f"{phlo_src_path}/../../pyproject.toml:/opt/phlo-dev/pyproject.toml:ro"
            )
            config["volumes"].append(pyproject_mount)
