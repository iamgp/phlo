...
Executes the identity and state decisions of ADR 0051 (Dataset authority
contract): canonical ID scheme (``candidate:<table_id>`` for candidates and
...
"""Payload schema version of the durable Dataset workflow collection (ADR 0051)."""
...
class WorkflowState(StrEnum):
    """Workflow states of a candidate Dataset (ADR 0051 decision table)."""
...
class PublicationState(StrEnum):
    """Publication states of a promoted Dataset (ADR 0051 decision table)."""
...
class CandidateRecord:
    ...
    publication state survive migration, per ADR 0051.
    """
...
class TransitionRequest:
    """Compare-and-set transition request (ADR 0051 idempotency contract).
...
class TransitionAuditEvent:
    """Append-only audit event for one transition attempt (ADR 0051)."""
