...
def discover_user_workflows(
    ...
    (which triggers @cascade_ingestion and @cascade_quality decorator registration),
...
def _collect_registered_assets() -> list[Any]:
    ...
    try:
        ...
        logger.warning("Could not import cascade.ingestion.get_ingestion_assets")
    except Exception as exc:
        logger.error(f"Error collecting ingestion assets: {exc}")

    # TODO: Collect quality check assets when registry is added
    # try:
    #     from cascade.quality import get_quality_assets
...
def _clear_asset_registries() -> None:
    ...
    """
    try:
        from cascade.ingestion.decorator import _INGESTION_ASSETS
    ...
    # try:
    #     from cascade.quality.decorator import _QUALITY_ASSETS
