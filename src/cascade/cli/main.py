...
def init(project_name: Optional[str], template: str, force: bool):
    ...
    Examples:
        cascade init my-data-project          # Create new project directory
        cascade init . --force                # Initialize in current directory
        cascade init weather-pipeline --template minimal
    """
    ...
    try:
        ...
        click.echo("  3. cascade create-workflow       # Create your first workflow")
        click.echo("  4. cascade dev                   # Start Dagster UI")
...
def dev(host: str, port: int, workflows_path: str):
    ...
    Examples:
        cascade dev                              # Start on localhost:3000
        cascade dev --port 8080                  # Use custom port
        cascade dev --workflows-path custom_workflows
    """
    ...
    if not Path("pyproject.toml").exists():
        ...
        click.echo("Initialize a new project with: cascade init", err=True)
        sys.exit(1)
...
name = "{project_name}"
version = "0.1.0"
description = "Cascade data workflows"
...
- [Cascade Documentation](https://github.com/iamgp/cascade)
- [Workflow Development Guide](https://github.com/iamgp/cascade/blob/main/docs/guides/workflow-development.md)

## Commands

- `cascade dev` - Start Dagster development server
- `cascade create-workflow` - Scaffold new workflow
- `cascade test` - Run tests
"""
