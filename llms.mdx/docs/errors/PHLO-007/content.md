# PHLO-007: Table Not Found (/docs/errors/PHLO-007)
...
2. **Wrong table name**
   ...
   * Snake\_case vs other naming mismatch
...
Solution 1: Verify the table exists in the catalog [#solution-1-verify-the-table-exists-in-the-catalog]
...
Solution 2: Run the ingestion asset first [#solution-2-run-the-ingestion-asset-first]
...
Solution 3: Check table naming [#solution-3-check-table-naming]

Phlo uses snake\_case for table names derived from asset names:
...
Solution 4: Verify catalog connection [#solution-4-verify-catalog-connection]
...
Incorrect: Wrong namespace [#incorrect-wrong-namespace]
...
Correct: Right namespace [#correct-right-namespace]

```python
table = catalog.load_table("bronze.dlt_weather_observations")
```

Incorrect: CamelCase table name [#incorrect-camelcase-table-name]

```python
table = catalog.load_table("default.WeatherObservations")
```

Correct: snake_case table name [#correct-snake_case-table-name]
