# PHLO-402: Iceberg Write Error (/docs/errors/PHLO-402)
...
Solution 1: Align data schema with table schema [#solution-1-align-data-schema-with-table-schema]
...
if extra:
    print(f" Extra columns (will be ignored): {extra}")
```

Solution 2: Fix column types [#solution-2-fix-column-types]
...
Solution 3: Check S3/MinIO access [#solution-3-check-s3minio-access]
...
Solution 4: Verify write permissions [#solution-4-verify-write-permissions]
...
Incorrect: Schema mismatch [#incorrect-schema-mismatch]
...
Correct: Matching schema [#correct-matching-schema]
