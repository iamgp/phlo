# Quality Checks Catalog (/docs/reference/quality-checks-catalog)
...
Table-Level Checks [#table-level-checks]
...
Advanced SQL Checks [#advanced-sql-checks]
...
| `check_type`         | `str`         | `"rowcount_parity"`      | Type: 'rowcount\_parity' (exact match) or 'rowcount\_gte' (target >= source) |
...
| -------------------- | ------------------- | ------------------------ | ------------------------------------------------------------------------ |
| `source_table`       | `str`               | Required                 | Fully qualified source table name                                        |
| `target_table`       | `str`               | Required                 | Fully qualified target table name (usually same as @phlo\_quality table) |
...
Decision Guide: @phlo_quality vs dbt tests vs pytest [#decision-guide-phlo_quality-vs-dbt-tests-vs-pytest]

Use @phlo_quality when: [#use-phlo_quality-when]
...
Use dbt tests when: [#use-dbt-tests-when]
...
Use pytest when: [#use-pytest-when]
...
1\. Start Simple, Add Gradually [#1-start-simple-add-gradually]
...
2\. Use Thresholds Wisely [#2-use-thresholds-wisely]
...
3\. Organize Checks by Layer [#3-organize-checks-by-layer]
...
4\. Use blocking Strategically [#4-use-blocking-strategically]
...
5\. Leverage Partition Awareness [#5-leverage-partition-awareness]
...
6\. Use Reconciliation Checks in Gold Layer [#6-use-reconciliation-checks-in-gold-layer]
...
7\. Document Check Purpose [#7-document-check-purpose]
...
8\. Monitor Check Performance [#8-monitor-check-performance]
...
9\. Sample Failures for Large Tables [#9-sample-failures-for-large-tables]
...
10\. Combine with Pandera Schemas [#10-combine-with-pandera-schemas]
