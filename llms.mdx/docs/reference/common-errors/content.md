# Common Errors and Solutions (/docs/reference/common-errors)



Top 10 Errors [#top-10-errors]

1\. Asset Not Found in Dagster UI [#1-asset-not-found-in-dagster-ui]
...
2\. unique_key Field Not in Schema [#2-unique_key-field-not-in-schema]
...
3\. Invalid Cron Expression [#3-invalid-cron-expression]
...
**Test your cron**: [https://crontab.guru/](https://crontab.guru/)

**Details**: [Error Message Audit](./audit/error_message_audit.md#scenario-3)

***

4\. Schema Validation Failed [#4-schema-validation-failed]
...
5\. Missing Required Schema Parameter [#5-missing-required-schema-parameter]
...
6\. DLT Pipeline Failed [#6-dlt-pipeline-failed]
...
7\. Iceberg Table Doesn't Exist [#7-iceberg-table-doesnt-exist]
...
8\. Docker Service Not Running [#8-docker-service-not-running]
...
9\. Import Error in Python Code [#9-import-error-in-python-code]
...
10\. Asset Shows in UI But Won't Materialize [#10-asset-shows-in-ui-but-wont-materialize]
...
| unique\_key not in schema | Field name mismatch     | Match field names exactly                                 |
| Invalid cron              | Wrong format            | Use standard cron format                                  |
| Missing schema            | No validation\_schema   | Add Pandera schema                                        |
...
Development Errors [#development-errors]
...
4. **Verify configuration**
   * Schema field names match unique\_key
...
1\. Validate Schema Early [#1-validate-schema-early]
...
2\. Match Field Names Exactly [#2-match-field-names-exactly]
...
3\. Test Cron Expressions [#3-test-cron-expressions]

Use [https://crontab.guru/](https://crontab.guru/) to validate cron expressions before using them.

4\. Use Environment Variables for Credentials [#4-use-environment-variables-for-credentials]
...
5\. Check Docker Services Before Materializing [#5-check-docker-services-before-materializing]
...
6\. Use Descriptive Asset Names [#6-use-descriptive-asset-names]
...
* **GitHub Discussions**: [https://github.com/phlohouse/phlo/discussions](https://github.com/phlohouse/phlo/discussions)
* **GitHub Issues**: [https://github.com/phlohouse/phlo/issues](https://github.com/phlohouse/phlo/issues)
