# DuckDB Iceberg Extension for Ad-hoc Analysis (/docs/reference/duckdb-queries)



Why DuckDB + Iceberg? [#why-duckdb--iceberg]
...
1\. Install DuckDB [#1-install-duckdb]
...
2\. Install Iceberg Extension [#2-install-iceberg-extension]
...
Option 1: Interactive Configuration (DuckDB CLI) [#option-1-interactive-configuration-duckdb-cli]
...
Option 2: Python Script Configuration [#option-2-python-script-configuration]
...
Option 3: Environment Variables (Recommended for Scripts) [#option-3-environment-variables-recommended-for-scripts]
...
Method 1: Direct S3 Path (Recommended) [#method-1-direct-s3-path-recommended]
...
Method 2: Nessie Catalog Integration (Advanced) [#method-2-nessie-catalog-integration-advanced]
...
3\. Aggregations and Analysis [#3-aggregations-and-analysis]
...
5\. Export to CSV [#5-export-to-csv]
...
1\. Use Partition Filters [#1-use-partition-filters]
...
2\. Limit Result Sets [#2-limit-result-sets]
...
3\. Use Aggregations [#3-use-aggregations]
...
Issue: Cannot connect to MinIO [#issue-cannot-connect-to-minio]
...
Issue: SSL/TLS errors [#issue-ssltls-errors]
...
Comparison: DuckDB vs Trino [#comparison-duckdb-vs-trino]
