# Hasura GraphQL Setup Guide (/docs/setup/hasura)
...
1\. Track Postgres Tables [#1-track-postgres-tables]

Hasura needs to "track" tables to generate GraphQL schema.

1. Open Hasura console: [http://localhost:10011/console](http://localhost:10011/console)
...
Configure `admin` Role [#configure-admin-role]
...
Configure `analyst` Role [#configure-analyst-role]
...
Repeat for All Tables [#repeat-for-all-tables]
...
3\. Test GraphQL Queries [#3-test-graphql-queries]
...
4\. Set Request Headers [#4-set-request-headers]
...
Actions (Custom Business Logic) [#actions-custom-business-logic]
...
Tables Not Showing [#tables-not-showing]
...
JWT Validation Failures [#jwt-validation-failures]
...
4. Test token at [https://jwt.io](https://jwt.io) (paste token, verify signature with secret)
