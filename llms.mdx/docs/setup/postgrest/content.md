# PostgREST Deployment Guide (/docs/setup/postgrest)
...
Step 3: Start PostgREST Service [#step-3-start-postgrest-service]
...
Step 4: Verify Health Check [#step-4-verify-health-check]
...
Test 1: Login and Get JWT Token [#test-1-login-and-get-jwt-token]
...
Test 2: Get Glucose Readings [#test-2-get-glucose-readings]
...
Test 3: Filter Glucose Readings [#test-3-filter-glucose-readings]
...
Test 4: Get Daily Summary [#test-4-get-daily-summary]
...
Test 5: Get Hourly Patterns [#test-5-get-hourly-patterns]
...
Test 6: Get Statistics (Function Call) [#test-6-get-statistics-function-call]
...
Test 7: Get Current User Info [#test-7-get-current-user-info]
...
Test 8: Test Authentication Errors [#test-8-test-authentication-errors]
...
Test 9: Get OpenAPI Documentation [#test-9-get-openapi-documentation]
...
Comparing with FastAPI [#comparing-with-fastapi]
...
Issue: "Connection refused" error [#issue-connection-refused-error]
...
Issue: "relation does not exist" error [#issue-relation-does-not-exist-error]
...
Issue: "JWT token invalid or expired" [#issue-jwt-token-invalid-or-expired]
...
Issue: "permission denied for schema api" [#issue-permission-denied-for-schema-api]
...
Issue: PostgREST container keeps restarting [#issue-postgrest-container-keeps-restarting]
...
Phase 2: Nginx Reverse Proxy (Optional) [#phase-2-nginx-reverse-proxy-optional]
...
Phase 4: Migration [#phase-4-migration]
...
URL Parameters (for GET requests) [#url-parameters-for-get-requests]
...
* **PostgREST API Reference**: [https://postgrest.org/en/stable/api.html](https://postgrest.org/en/stable/api.html)
* **PostgreSQL RLS Guide**: [https://www.postgresql.org/docs/current/ddl-rowsecurity.html](https://www.postgresql.org/docs/current/ddl-rowsecurity.html)
