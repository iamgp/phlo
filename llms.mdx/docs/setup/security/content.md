# Security Setup Guide (/docs/setup/security)
...
Option 1: LDAP Authentication [#option-1-ldap-authentication]

LDAP works with Trino and MinIO. Configure your LDAP server details:

Trino LDAP [#trino-ldap]
...
Option 2: OAuth2/OIDC Authentication [#option-2-oauth2oidc-authentication]
...
Keycloak Example Setup [#keycloak-example-setup]
...
Nessie Authorization [#nessie-authorization]
...
MinIO IAM Policies [#minio-iam-policies]
...
MinIO Server-Side Encryption [#minio-server-side-encryption]
...
MinIO Audit Logs [#minio-audit-logs]
...
PostgreSQL Audit Logging [#postgresql-audit-logging]
...
Trino Query Logging [#trino-query-logging]
...
PostgREST JWT Authentication [#postgrest-jwt-authentication]
...
Observatory Token Auth [#observatory-token-auth]
...
OIDC Token Issues [#oidc-token-issues]
...
LDAP Connection Issues [#ldap-connection-issues]
