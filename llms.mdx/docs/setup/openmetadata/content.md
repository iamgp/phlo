# OpenMetadata Data Catalog Setup Guide (/docs/setup/openmetadata)
...
Why OpenMetadata for Phlo? [#why-openmetadata-for-phlo]
...
1\. Start OpenMetadata Services [#1-start-openmetadata-services]
...
2\. Access OpenMetadata UI [#2-access-openmetadata-ui]
...
3\. Complete Setup Checklist [#3-complete-setup-checklist]
...
1. Navigate to [http://localhost:10020](http://localhost:10020)
2. Login with `admin` / `admin`
3. Complete the welcome tour (optional)
4. You'll see the empty catalog dashboard

Connecting Phlo Data Sources [#connecting-phlo-data-sources]

Step 1: Add Trino Database Service [#step-1-add-trino-database-service]
...
Step 2: Configure Trino Connection [#step-2-configure-trino-connection]
...
Step 3: Configure Metadata Ingestion Pipeline [#step-3-configure-metadata-ingestion-pipeline]
...
Step 4: Configure Scheduling [#step-4-configure-scheduling]
...
Step 6: Run Initial Ingestion [#step-6-run-initial-ingestion]
...
Step 7: Enable Search (CRITICAL) [#step-7-enable-search-critical]
...
2. Click on **trino**
3. Navigate: iceberg → raw → glucose\_entries
4. You should see:
...
Complete Ingestion Configuration [#complete-ingestion-configuration]
...
Editing Configuration Later [#editing-configuration-later]
...
Marts (BI-Ready) [#marts-bi-ready]

* `marts.glucose_analytics_mart` - Published to Postgres for Superset

Using the Data Catalog [#using-the-data-catalog]
...
Enable Lineage Tracking with dbt [#enable-lineage-tracking-with-dbt]
...
Integration with Phlo Workflows [#integration-with-phlo-workflows]

Update Ingestion Schedule [#update-ingestion-schedule]
...
Document in dbt Models [#document-in-dbt-models]
...
OpenMetadata UI Not Loading [#openmetadata-ui-not-loading]
...
Search Not Working / "All Shards Failed" Error [#search-not-working--all-shards-failed-error]

**Symptom:**

* Explore page shows: "Search failed due to Elasticsearch exception \[type=search\_phase\_execution\_exception, reason=all shards failed]"
...
Missing Elasticsearch Indices [#missing-elasticsearch-indices]
...
Server Crashes During Ingestion [#server-crashes-during-ingestion]
...
Elasticsearch Out of Memory [#elasticsearch-out-of-memory]
...
Trino Connection Failed [#trino-connection-failed]
...
Ingestion Pipeline Failing [#ingestion-pipeline-failing]
...
Tables Not Appearing After Ingestion [#tables-not-appearing-after-ingestion]
...
| Active Development | Active       | Slowed   | Very Active  |
...
Related Phlo Documentation [#related-phlo-documentation]
