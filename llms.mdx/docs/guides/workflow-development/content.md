...
3. OpenWeather API key (free at [https://openweathermap.org/api](https://openweathermap.org/api))
...
1.1 Create Schema Definition [#11-create-schema-definition]
...
1.2 Create Iceberg Table Schema (Optional) [#12-create-iceberg-table-schema-optional]
...
Step 2: Create the Ingestion Asset [#step-2-create-the-ingestion-asset]

Now let's fetch data from the OpenWeather API and store it in Iceberg.

2.1 Add API Configuration [#21-add-api-configuration]
...
2.2 Create the Ingestion Asset [#22-create-the-ingestion-asset]
...
Step 3: Create Bronze Layer (Staging) [#step-3-create-bronze-layer-staging]

The Bronze layer cleans and standardizes raw data.

3.1 Define the Source [#31-define-the-source]
...
3.2 Create the Staging Model [#32-create-the-staging-model]
...
Step 4: Create Silver Layer (Facts) [#step-4-create-silver-layer-facts]

The Silver layer adds business logic and calculated fields.

4.1 Create the Fact Table [#41-create-the-fact-table]
...
Step 5: Create Gold Layer (Aggregations) [#step-5-create-gold-layer-aggregations]

The Gold layer creates aggregated, business-ready datasets.

5.1 Create Daily Summary [#51-create-daily-summary]
...
5.2 Create Location Dimension [#52-create-location-dimension]
...
Step 6: Create Marts for BI [#step-6-create-marts-for-bi]

Marts are optimized for BI tools and end-user queries.

6.1 Create Weather Overview Mart [#61-create-weather-overview-mart]
...
6.2 Create Recent Readings Mart [#62-create-recent-readings-mart]
...
Step 7: Add Data Quality Checks [#step-7-add-data-quality-checks]

Quality checks ensure your data meets expectations.

7.1 Create Quality Check Asset [#71-create-quality-check-asset]
...
7.2 Quality Check Discovery [#72-quality-check-discovery]

No manual registration is needed. Phlo discovers quality workflows under `workflows/quality/`.

***

Step 8: Configure Publishing [#step-8-configure-publishing]

Configure which tables get published to PostgreSQL for BI tools.

8.1 Update Publishing Config [#81-update-publishing-config]
...
8.2 Create Publishing Asset [#82-create-publishing-asset]
...
Step 9: Add Scheduling [#step-9-add-scheduling]

Schedule your pipeline to run automatically.

9.1 Create Schedule [#91-create-schedule]
...
9.2 Add Sensor for Freshness [#92-add-sensor-for-freshness]
...
10.1 Run Complete Pipeline [#101-run-complete-pipeline]
...
10.2 Verify Each Layer [#102-verify-each-layer]
...
10.3 Run dbt Tests [#103-run-dbt-tests]
...
10.4 Create Superset Dashboard [#104-create-superset-dashboard]

1. Open Superset: [http://localhost:10008](http://localhost:10008)
...
Pattern 1: Incremental Processing [#pattern-1-incremental-processing]
...
Pattern 2: Partitioned Assets [#pattern-2-partitioned-assets]
...
Pattern 3: Dynamic Cities [#pattern-3-dynamic-cities]
