# Compose Generation (/docs/guides/compose-generation)
...
env.py — environment variable materialization [#envpy--environment-variable-materialization]
...
generator.py — compose YAML generation [#generatorpy--compose-yaml-generation]
...
2. Builds per-service configs — image/build, ports, volumes, healthchecks, env\_file.
...
native.py — native subprocess services [#nativepy--native-subprocess-services]
...
Service plugin contributions [#service-plugin-contributions]
...
ServiceDiscovery cache APIs [#servicediscovery-cache-apis]
...
Customization via phlo.yaml [#customization-via-phloyaml]
