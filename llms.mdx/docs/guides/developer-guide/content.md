# Developer Guide (/docs/guides/developer-guide)
...
@phlo_ingestion Decorator [#phlo_ingestion-decorator]
...
DLT Source Integration [#dlt-source-integration]
...
Pandera Schemas [#pandera-schemas]

Schemas serve as the source of truth for data structure and validation.

Schema Definition Approaches [#schema-definition-approaches]
...
Approach 1: Manual Schema Definition [#approach-1-manual-schema-definition]
...
Approach 2: Generate Schemas from dbt YAML [#approach-2-generate-schemas-from-dbt-yaml]

**Reduce schema duplication by 50%** - define your schema once in dbt YAML, auto-generate the Pandera schema.

Why Use dbt YAML Generation? [#why-use-dbt-yaml-generation]
...
* dbt data\_tests automatically become Pandera Field constraints
* Works seamlessly with `@phlo_pandera` decorator

Step 1: Define Schema in dbt YAML [#step-1-define-schema-in-dbt-yaml]
...
Step 2: Generate Pandera Schema [#step-2-generate-pandera-schema]
...
Step 3: Use with @phlo_pandera [#step-3-use-with-phlo_pandera]
...
When to Use dbt YAML Generation vs Manual Schemas [#when-to-use-dbt-yaml-generation-vs-manual-schemas]

**Use dbt YAML generation when**:

* You have dbt transformations defining the schema
* Schema is relatively simple (standard data\_tests)
...
Schema Conversion to Iceberg [#schema-conversion-to-iceberg]
...
@phlo_pandera Decorator [#phlo_pandera-decorator]
...
Reconciliation Checks (Cross-table) [#reconciliation-checks-cross-table]
...
**Common reconciliation gaps (use CustomSQLCheck or @asset\_check):**
...
Advanced Quality Checks [#advanced-quality-checks]
...
Publishing to BI Tools [#publishing-to-bi-tools]

Automatically publish Iceberg marts to PostgreSQL for BI tools.

Publishing Asset [#publishing-asset]
...
Generic Publisher [#generic-publisher]
...
1\. Schema-First Development [#1-schema-first-development]
