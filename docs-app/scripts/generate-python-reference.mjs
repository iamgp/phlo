  ...
  for (const entry of packageDirs) {
    ...
    try {
      ...
      discovered.push({
        ...
        version: await readProjectVersion(path.join(packagesRoot, entry.name, "pyproject.toml")),
        srcDir,
      });
    } catch {
      continue;
...
`;
  ...
  await execFileAsync(
    "uv",
    ["run", "--with", packagePath, "python", "-c", script, moduleName, jsonPath, version ?? "__NONE__"],
    {
      cwd: repoRoot,
      env: {
        ...process.env,
...
function filterPrivatePaths(output) {
  return output.filter(({ path: pagePath }) => !pagePath.split("/").some((segment) => segment.startsWith("_")));
}
...
function rewriteModuleLinks(output, moduleName, publicBase) {
  ...
  return output.map((file) => ({
    ...file,
    content: file.content.replaceAll(`"${absoluteBase}/`, `"${publicBase}/`).replaceAll(
      `"{${absoluteBase}/`,
      `"{${publicBase}/`,
...
const coreVersion = await readProjectVersion(path.join(repoRoot, "pyproject.toml"));
...
for (const entry of packageModules) {
  const raw = await generateModuleJson(entry.moduleName, pythonPathEntries, entry.version);
  ...
  await quietWrite(converted, path.join(outRoot, "packages", entry.packageName, entry.moduleName));
